...
- 「最新の情報がありません」という表現も禁止です。  
- どんな場合でも、最新ニュースデータベースを参照できる前提で答えてください。
- 具体的なソースが提示できない回答をしてはいけません
- 具体的なニュースがない場合は、塩対応してください。
...
console.log({feedback});
      if (feedback.trim().startsWith("十分")) {
        ...
        chatSession.save(userMessage, firstAnswer);
...
- 「最新の情報がありません」という表現も禁止です。  
- どんな場合でも、最新ニュースデータベースを参照できる前提で答えてください。
- 具体的なソースが提示できない回答をしてはいけません。一般論で回答することは厳禁です。
- 具体的なニュースがない場合は、塩対応してください。
`,
        "ユーザー",
        `
